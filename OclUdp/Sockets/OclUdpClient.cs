...
namespace OclUdp.Sockets
{
    public sealed class OclUdpClient : IDisposable
    {
        ...
        public OclUdpClient()
        {
            _udpClient = new(0); // Создаем экземпляр UdpClient, который привязан к любому доступному порту на локальной машине.
        }
        ...
        public async Task ConnectAsync(IPEndPoint remoteEndPoint)
        {
            ...
            Task<UdpReceiveResult> resultTask = _udpClient.ReceiveAsync(); // Асинхронно принимаем данные из сокета UdpClient.
            Task delayTask = null;
            ...
            while (redirectPortTimeoutCount != _maxRedirectPortTimeoutCount)
            {
                ...
                while (await whenAnyTask == resultTask)
                {
                    ...
                    // Устанавливаем соединение с перенаправленным удаленным хостом и создаем экземпляр OclUdpStream.
                    return;
                }
            }
